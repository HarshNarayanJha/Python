...
int bellTimingsMinutes[4] = {15, 40, 45, 55};

// Constant length for assurance of correct mapping
const int bellTimingsLen = 12;

// Array of bell ring types => (cotinued long or n times)
...
void loop() {
	...
	if (indexOfInt(bellTimingsHours, currentHours) != -1) {
		if (indexOfInt(bellTimingsMinutes, currentMinutes) != -1) {
			// It is bell time !!!
			ringBell();
		}
	}
...
// Used for finding the index of an int element in an array
...
// Used for finding the index of an array in another 2-dimensional array with the length of inner array = 2
int indexOfArray(int ar[][2], int elem[]) {
	int index = -1;
	int len = sizeof(ar) / sizeof(ar[0]);
	for (int i = 0; i < len; ++i)
	{
...
void ringBell() {
	...
	int index = indexOfArray(bellTimings, hmArray);		// get the index of the constructed array in the (bellTimings) array
...
/* Last Updated: 03-10-2021 at 07:01 pm */
